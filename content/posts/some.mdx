...
> checcheck outcheckoutcheckoutcheckout checkoutcheckoutcheckoutche ckoutcheckoutcheckoutcheckou
> tcheckoutcheckoutcheckoutchecx koutcheckoutcheckoutcheckout
...
6.  **hostname:** represent the hostname without the port number.\
    Example: `example.com`
7.  **pathname**: Represents the path of the URL, which comes after the host.

    \Example: `/test/foo`

8.  **search:** Represents the query parameters of the URL, starting with a question mark.\
    Example: `?query=medallion`
9.  **searchParams**: Represents a URLSearchParams object, which allows easy manipulation of query parameters.\
    Example: `URLSearchParams { 'query' => 'medallion' }`
10. **hash**: Represents the fragment identifier of the URL, starting with a hash (#). (empty in our case).
